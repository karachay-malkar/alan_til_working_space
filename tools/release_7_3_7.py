...
def patch_tests():
    ...
    p.write_text(s,encoding='utf-8')
    p=ROOT/'tests/map-smoke.mjs'; s=p.read_text().replace('7.3.6','7.3.7').replace(r'7\.3\.6',r'7\.3\.7'); marker="  await page.evaluate(() => window.ALAN_MAP_INSTANCE.map.jumpTo({zoom:7}));\n"
